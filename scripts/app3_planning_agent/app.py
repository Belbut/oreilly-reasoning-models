...
def generate_plan(client: OpenAI, goal: str, model: str, effort: str) -> tuple[Optional[Plan], str, str]:
    """
    Generate a structured plan for the given goal using GPT-5.6 with reasoning.
...
def generate_critique(client: OpenAI, plan: Plan, model: str, effort: str) -> tuple[Optional[Critique], str, str]:
    """
    Generate a critique of the given plan using GPT-5.6 with reasoning.
...
def main():
    ...
    with st.sidebar:
        ...
        # Model selector (GPT-5.6 only for reasoning)
        model = st.selectbox(
            "Model",
            ["gpt-5.6"],
            index=0,
            help="GPT-5.6 with adaptive reasoning"
        )
