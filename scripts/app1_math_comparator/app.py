...
Compares GPT-5.6 with reasoning=none (standard) against GPT-5.6 with reasoning=high,
...
def call_standard_model(client: OpenAI, problem: str) -> dict:
    """
    Call GPT-5.6 with reasoning effort=none (fast, pattern-based response).
...
def call_reasoning_model(client: OpenAI, problem: str, effort: str = "high") -> dict:
    """
    Call GPT-5.6 with reasoning effort enabled (slower, with chain-of-thought).
...
def main():
    ...
    # Model info
    st.sidebar.markdown("**Model:** `gpt-5.6`")
