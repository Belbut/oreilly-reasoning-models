...
Uses OpenAI's GPT-5.6 with reasoning effort via the Responses API.
"""
...
def solve_puzzle(puzzle: dict, reasoning_effort: str = "high") -> tuple[dict, str, float, dict]:
    """
    Solve a puzzle using GPT-5.6 with reasoning effort.
    ...
    # Use GPT-5.6 with reasoning effort and summary
...
def main():
    ...
    st.markdown("*Powered by OpenAI GPT-5.6 with reasoning capabilities*")
    ...
    reasoning_effort = st.sidebar.select_slider(
        ...
        value="high",
        help="Controls how much 'thinking' GPT-5.6 does (none=fastest, xhigh=most thorough)"
    )
    ...
    st.markdown("---")
    st.caption(
        "Built with Streamlit and OpenAI GPT-5.6 | "
